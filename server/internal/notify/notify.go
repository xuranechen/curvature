...
func BuildSessionPayload(in SessionNotification) Payload {
	...
	root := firstNonEmpty(in.RootTitle, in.RootID, "Curvature")
	...
	tag := fmt.Sprintf("curvature:%s:%s:%s", kind, in.RootID, in.SessionKey)
	eventID := firstNonEmpty(in.EventID, tag)
	if kind == "session.done" {
		tag = fmt.Sprintf("curvature:%s:%s:%s:%s", kind, in.RootID, in.SessionKey, eventID)
	}
	return Payload{
		Type:               kind,
		Title:              title,
...
func BuildScheduledPayload(in ScheduledNotification) Payload {
	...
	tag := fmt.Sprintf("curvature:%s:%s:%s", kind, in.RootID, in.TaskID)
	return Payload{
		Type:               kind,
