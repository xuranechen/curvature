...
func TestBuildSessionPayloadMatchesNativeReplyShape(t *testing.T) {
	...
	if payload.Title != "Curvature Repo · 实现通知 · 完成" {
...
func TestBuildScheduledPayload(t *testing.T) {
	...
	if done.Title != "Curvature · 定时任务完成" || done.Body != "每日总结: 运行完成" {
	...
	if failed.Title != "Curvature · 定时任务失败" || failed.Body != "每日总结: agent unavailable" {
...
func TestTestPayloadDoesNotUseFixedNotificationTag(t *testing.T) {
	payload := testPayload()
	if payload.Type != "test" || payload.Title != "Curvature" {
