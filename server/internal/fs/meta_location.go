...
const homeMetaIdentityFile = ".curvature-project.json"
...
// project-local .curvature directory always wins over the user's default.
func MetaLocationForNewRoot(rootPath, preferred string) (string, error) {
	...
	if lstatErr == nil {
		...
		if !info.IsDir() {
			return "", errors.New("project .curvature path is not a directory")
		}
		return MetaLocationProject, nil
	}
