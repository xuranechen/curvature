...
func TestMetaLocationForNewRootPrefersExistingProjectMetadata(t *testing.T) {
	rootDir := t.TempDir()
	if err := os.Mkdir(filepath.Join(rootDir, ".curvature"), 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestHomeMetadataReusesSameAbsolutePathAndRejectsDifferentPath(t *testing.T) {
	...
	read, err := readded.ReadFile(".curvature/state.json", 0, 0, "full")
	if err != nil {
		t.Fatal(err)
	}
	if read.Path != ".curvature/state.json" || !strings.Contains(read.Content, `"ok"`) {
...
func TestEmptyMetaLocationDefaultsToProject(t *testing.T) {
	...
	if root.MetaDir() != filepath.Join(rootDir, ".curvature") {
...
func TestEnsureMetaDirAddsGitExcludeEntry(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(rootDir, ".curvature")); err != nil {
...
func TestRegistryRenameMovesHomeMetadataAndUpdatesIdentity(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(home, ".curvature", "old-project")); !os.IsNotExist(err) {
...
func TestRootInfoNormalizePathAcceptsAbsolutePathWithoutLeadingSlash(t *testing.T) {
	root := NewRootInfo("curvature", "curvature", "/Users/bixin/project/curvature")

	got, err := root.NormalizePath("Users/bixin/project/curvature/test.json")
	if err != nil {
...
func TestRootInfoNormalizePathStripsFragment(t *testing.T) {
	...
	got, err := root.NormalizePath("Users/bixin/project/curvature/design/test.md#L89")
	if err != nil {
		t.Fatalf("NormalizePath returned error: %v", err)
	}
...
func TestRootInfoListEntriesIncludesSizeAndMTime(t *testing.T) {
	...
	root := NewRootInfo("curvature", "curvature", rootDir)
...
func TestSharedFileWatcherShouldIgnoreLargeGeneratedDirectories(t *testing.T) {
	...
	tests := []struct {
		...
		{filepath.Join(rootDir, ".curvature"), true},
		{filepath.Join(rootDir, ".curvature", "state.json"), true},
		{filepath.Join(rootDir, ".curvature2"), false},
