...
func cleanSystemHistoryCommand(command string) string {
	...
	if command == "" || isCurvatureControlHistoryCommand(command) {
		return ""
	}
	if unwrapped, ok := unwrapCurvatureEvalHistoryCommand(command); ok {
		return strings.TrimSpace(unwrapped)
	}
	return command
}

func isCurvatureControlHistoryCommand(command string) bool {
	...
	return strings.HasPrefix(lower, "command printf ") ||
		...
		strings.HasPrefix(lower, "echo __curvature_cmd_") ||
		strings.HasPrefix(lower, "set -l __curvature_status ") ||
		strings.HasPrefix(lower, "prompt $s") ||
		strings.HasPrefix(lower, "chcp 65001 ")
}

func unwrapCurvatureEvalHistoryCommand(command string) (string, bool) {
