...
func (s *longShellSession) run(_ context.Context, command string) (*longShellCommand, error) {
	run := &longShellCommand{
		...
		startMarker: "__CURVATURE_CMD_START__",
	}
	run.startMarker = "__CURVATURE_CMD_START_" + run.id + "__"
	run.endPrefix = "__CURVATURE_CMD_END_" + run.id + "__:"
...
func isPowerShellControlEchoLine(line string) bool {
	...
	if strings.Contains(text, "[Console]::OutputEncoding") ||
		...
		strings.Contains(text, "$__curvature_status") ||
...
func shellControlEchoIndex(line string) int {
	...
	tokens := []string{
		"__CURVATURE_CMD_START_",
		"__CURVATURE_CMD_END_",
		"__curvature_status",
		"$global:LASTEXITCODE",
