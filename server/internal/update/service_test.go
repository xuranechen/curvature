...
func TestParseReleaseNotesVersion(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "tag heading", text: "# Curvature v0.2.3\n\n## Fixes\n", want: "v0.2.3"},
		{name: "version without prefix", text: "# Curvature 0.2.3\n", want: "0.2.3"},
...
func TestLatestReleaseNotesBody(t *testing.T) {
	...
	text := "# Curvature v0.2.3\n\n## 优化和修复\n- latest\n\n# Curvature v0.2.2\n\n## 修复\n- old\n"
	want := "# Curvature v0.2.3\n\n## 优化和修复\n- latest"
...
func TestFetchAndVerifyManifest(t *testing.T) {
	...
	payload := []byte(`{"version":"v1.2.3","repo":"a9gent/curvature","artifacts":[{"name":"curvature_v1.2.3_linux_amd64.tar.gz","sha256":"` + strings.Repeat("a", 64) + `","size":123}]}` + "\n")
	body := signedManifestBody(t, payload, ed25519.Sign(privateKey, payload))
	manifestURL := "https://github.com/a9gent/curvature/releases/download/v1.2.3/curvature_v1.2.3_manifest.json"
	service := NewService("a9gent/curvature", "v1.2.2", "/tmp/bin/curvature", nil, time.Hour)
	...
	if got := manifest.Artifacts[0].Name; got != "curvature_v1.2.3_linux_amd64.tar.gz" {
...
func TestRelayAssetURL(t *testing.T) {
	name := "curvature_v0.3.4_windows_amd64.zip"
	want := "https://relay.a9gent.com/curvature-downloads/curvature_v0.3.4_windows_amd64.zip"
	if got := relayAssetURL(name); got != want {
		t.Fatalf("relayAssetURL() = %q, want %q", got, want)
	}
	for _, name := range []string{"", "../curvature.zip", `dir\curvature.zip`} {
...
func TestInstallLayoutInstalled(t *testing.T) {
	service := NewService("a9gent/curvature", "v1.2.2", filepath.Join("opt", "curvature", "bin", "curvature"), nil, time.Hour)
	...
	if got, want := filepath.Clean(layout.Prefix), filepath.Join("opt", "curvature"); got != want {
		t.Fatalf("layout prefix = %q, want %q", got, want)
	}
	bin, agents, taskTemplate, web := layout.destinationPaths("curvature")
	if got, want := bin, filepath.Join("opt", "curvature", "bin", "curvature"); got != want {
		t.Fatalf("bin path = %q, want %q", got, want)
	}
	if got, want := agents, filepath.Join("opt", "curvature", "share", "curvature", "agents.json"); got != want {
		t.Fatalf("agents path = %q, want %q", got, want)
	}
	if got, want := taskTemplate, filepath.Join("opt", "curvature", "share", "curvature", "task_template.json"); got != want {
		t.Fatalf("task template path = %q, want %q", got, want)
	}
	if got, want := web, filepath.Join("opt", "curvature", "share", "curvature", "web"); got != want {
		t.Fatalf("web path = %q, want %q", got, want)
	}
}

func TestInstallLayoutPortable(t *testing.T) {
	service := NewService("a9gent/curvature", "v1.2.2", filepath.Join("tmp", "curvature_v1.2.2_linux_amd64", "curvature"), nil, time.Hour)
	layout, err := service.installLayout()
	if err != nil {
		t.Fatalf("installLayout() error = %v", err)
	}
	...
	if got, want := filepath.Clean(layout.ExeDir), filepath.Join("tmp", "curvature_v1.2.2_linux_amd64"); got != want {
		t.Fatalf("layout exe dir = %q, want %q", got, want)
	}
	bin, agents, taskTemplate, web := layout.destinationPaths("curvature")
	if got, want := bin, filepath.Join("tmp", "curvature_v1.2.2_linux_amd64", "curvature"); got != want {
		t.Fatalf("bin path = %q, want %q", got, want)
	}
	if got, want := agents, filepath.Join("tmp", "curvature_v1.2.2_linux_amd64", "agents.json"); got != want {
		t.Fatalf("agents path = %q, want %q", got, want)
	}
	if got, want := taskTemplate, filepath.Join("tmp", "curvature_v1.2.2_linux_amd64", "task_template.json"); got != want {
		t.Fatalf("task template path = %q, want %q", got, want)
	}
	if got, want := web, filepath.Join("tmp", "curvature_v1.2.2_linux_amd64", "web"); got != want {
		t.Fatalf("web path = %q, want %q", got, want)
	}
}
...
func TestSafeArchiveTargetRejectsTraversal(t *testing.T) {
	...
	if target, err := safeArchiveTarget(root, "curvature_v1.2.3/curvature"); err != nil {
