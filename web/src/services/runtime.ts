...
export function getNativePlatform(): NativePlatform {
  ...
  const bridgePlatform = String(win.CurvatureNative?.platform || "").toLowerCase();
  if (bridgePlatform === "harmony" || bridgePlatform === "android") {
    return bridgePlatform;
  }
  if (win.CurvatureHarmony) {
    return "harmony";
  }
...
function readStorage(key: string): string {
  if (key === "curvature_api_base_url") {
    return sanitizeBaseURL(getStoredApiBaseURL());
  }
  if (key === "curvature_ws_base_url") {
...
export function getApiBaseURL(): string {
  const configured = readStorage("curvature_api_base_url") || readMeta("curvature-api-base-url");
  if (configured) {
    return configured;
  }
...
export function getWsBaseURL(): string {
  const configured = readStorage("curvature_ws_base_url") || readMeta("curvature-ws-base-url");
  if (configured) {
    return configured;
  }
