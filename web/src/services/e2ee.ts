...
const SECRET_STORAGE_PREFIX = "curvature.e2ee.secret.";
export const E2EE_HEADER = "X-Curvature-E2EE";
export const CLIENT_ID_HEADER = "X-Curvature-Client-ID";
export const PROOF_HEADER = "X-Curvature-Proof";
export const TS_HEADER = "X-Curvature-TS";
...
async function buildOpenProof(secret: string, nodeId: string, clientEphPK: string, clientNonce: string): Promise<string> {
  return buildHmacProof(secret, "curvature-e2ee-open", [nodeId, clientEphPK, clientNonce]);
}
...
async function buildAcceptProof(secret: string, nodeId: string, clientEphPK: string, nodeEphPK: string, clientNonce: string, serverNonce: string): Promise<string> {
  return buildHmacProof(secret, "curvature-e2ee-accept", [nodeId, clientEphPK, nodeEphPK, clientNonce, serverNonce]);
}
...
async function buildRequestProof(key: Uint8Array, method: string, path: string, ts: string, clientId: string): Promise<string> {
  ...
  const digest = await sha256Bytes(["curvature-request-proof", method, path, ts, clientId].join("\x1f"));
