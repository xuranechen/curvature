...
export function buildVersionEndpoint(platform: NativePlatform): string {
  if (platform === "harmony") {
...
async function getAppInfo(): Promise<{ version?: string; build?: string }> {
  ...
  if (isAndroidRuntime()) {
    const nativeBridge = (window as WindowWithAppInfoBridge).CurvatureAppInfo;
...
function defaultPackageFilename(platform: NativePlatform): string {
  return platform === "harmony" ? "curvature-harmony.hap" : "curvature-android.apk";
}
