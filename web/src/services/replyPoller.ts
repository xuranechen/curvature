...
type NativeReplyPollerSyncWindow = Window & {
  CurvatureReplyPoller?: NativeReplyPollerBridge;
  __curvatureLatestReplyPollerConfig?: ReplyPollerConfigPayload;
};
...
export async function syncNativeReplyPollerE2EE(): Promise<void> {
  ...
  const bridge = (window as NativeReplyPollerSyncWindow).CurvatureReplyPoller;
...
function rememberLatestReplyPollerConfig(payload: ReplyPollerConfigPayload): void {
  ...
  (window as NativeReplyPollerSyncWindow).__curvatureLatestReplyPollerConfig = payload;
}
...
function isLocalShellURL(value: string): boolean {
  try {
    ...
    if (isHarmonyRuntime()) {
      return url.hostname === "curvature.local";
    }
