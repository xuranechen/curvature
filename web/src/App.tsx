...
const MULTI_PROJECT_SESSION_STORAGE_KEY = "curvature-multi-project-session-list";
const APP_DOCUMENT_TITLE = "Curvature";
...
const RELAY_LAST_NODE_ID_STORAGE_KEY = "curvature.relay.last_node_id";
const PLUGIN_QUERY_STORAGE_PREFIX = "vp-progress:";
const TREE_SORT_STORAGE_KEY = "curvature-tree-sort-mode";
const DIRECTORY_SORT_OVERRIDES_STORAGE_KEY = "curvature-directory-sort-overrides";
const FILE_SCROLL_STORAGE_KEY = "curvature-file-scroll-positions";
const LAST_ROOT_STORAGE_KEY = "curvature-last-root-id";
const GIT_STATUS_EXPANDED_STORAGE_KEY = "curvature-git-status-expanded";
const GIT_HISTORY_EXPANDED_STORAGE_KEY = "curvature-git-history-expanded";
const TASK_TEMPLATE_SELECTION_STORAGE_KEY = "curvature-task-template-selection";
...
const MOBILE_ENTER_KEY_SEND_STORAGE_KEY = "curvature-mobile-enter-key-sends";
const SIDEBARS_SWAPPED_STORAGE_KEY = "curvature-sidebars-swapped";
const GIT_DIFF_SIDE_BY_SIDE_STORAGE_KEY = "curvature-git-diff-side-by-side";
const TASK_CREATE_WORKTREE_PREF_STORAGE_KEY = "curvature-task-create-worktree-pref";
const MAIN_CONTENT_VIEW_STORAGE_KEY = "curvature-main-content-view";
const DEFAULT_MAIN_CONTENT_VIEW_STORAGE_KEY = "curvature-default-main-content-view";
...
export function App({ onGoHome }: AppProps) {
  ...
  const handleStartUpdate = useCallback(async () => {
    ...
    if (next.has_update && status === "available") {
      ...
      if (
        !window.confirm(
          `Install ${target} now? Curvature will restart after the update finishes.`,
        )
      ) {
        return;
      }
  ...
  const handleSessionChipClick = useCallback(
    ...
    window.addEventListener("curvature:open-reply-session", handleOpenReplySession);
    openReplySession((window as any).__curvaturePendingReplySession);
    delete (window as any).__curvaturePendingReplySession;
    return () => {
      window.removeEventListener("curvature:open-reply-session", handleOpenReplySession);
    };
    ...
    const handleFileChangedBatch = (payload: any) => {
      ...
      if (
        [...paths, ...dirs].some((path) => {
          ...
          return normalized === ".curvature/plugins" || normalized.startsWith(".curvature/plugins/");
        })
      ) {
  ...
  kanbanStageColumns.sort((a, b) => a.index - b.index);
	  const kanbanTaskPanel = currentRootId ? (
        ...
        >
          ...
          >
            <SyncIcon
              style={kanbanRefreshSpin.refreshing ? { animation: "curvature-update-spin 0.8s linear infinite" } : undefined}
            />
          </span>
        </button>
    ...
    workspaceView = (
      <div
        style={{
          ...
          justifyContent: "center",
          padding: 24,
          background: "var(--curvature-main-bg, transparent)",
        }}
      >
        <section
          style={{
            ...
            border: "1px solid var(--border-color)",
            borderRadius: 8,
            background: "var(--curvature-panel-bg, var(--bg-primary))",
            padding: 20,
            display: "flex",
            flexDirection: "column",
            gap: 14,
        ...
        >
          <div
            style={{
              ...
              alignItems: "center",
              justifyContent: "space-between",
              background: "var(--curvature-topbar-bg, transparent)",
              fontSize: 12,
              color: "var(--text-secondary)",
            }}
          >
  ...
  // The single modal Save button applies the relay base URL, node name and
  // access password together. The name and password may also be saved before
  // the node is bound: they are persisted locally and synced to the relay on
  // bind or reconnect.
  const handleSaveRelaySettings = useCallback(async () => {
    setRelayConfigBusy(true);
    setRelayConfigError("");
    try {
      await bootstrapService.configureRelayBaseURL(relayBaseInput);
      const status = await bootstrapService.setRelayNodeSettings(
        relayNodeNameInput,
        relayPasswordInput,
      );
      if (relayNodeNameInput.trim()) {
      ...
      setRelayPasswordInput("");
      setRelaySettingsOpen(false);
    } catch (error) {
      setRelayConfigError(
        error instanceof Error ? error.message : t("relay.configFailed"),
      );
      void bootstrapService
        .refreshRelayStatus()
        .catch(() => undefined);
    } finally {
      setRelayConfigBusy(false);
    }
  }, [relayBaseInput, relayNodeNameInput, relayPasswordInput, t]);
        ...
        >
          ...
          >
            <div style={{ fontSize: "20px", fontWeight: 700, color: "#0f172a" }}>
              {t("relay.settingsTitle")}
            </div>

            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              <label
                htmlFor="relay-base-url-input"
                style={{ fontSize: "13px", fontWeight: 600, color: "#334155" }}
              >
                {t("relay.baseUrlLabel")}
              </label>
              <input
                id="relay-base-url-input"
                type="text"
                value={relayBaseInput}
                onChange={(event) => {
                  setRelayBaseInput(event.target.value);
                  if (relayConfigError) {
                    setRelayConfigError("");
                  }
                }}
                onKeyDown={(event) => {
                  if (event.key === "Enter" && !relayConfigBusy) {
                    void handleSaveRelaySettings();
                  }
                }}
                placeholder={t("relay.baseUrlPlaceholder")}
                disabled={relayConfigBusy}
                style={{
                  padding: "10px 12px",
                  borderRadius: "10px",
                  border: "1px solid var(--border-color)",
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              ...
              <div style={{ fontSize: "13px", color: "#0f172a" }}>
                {relayStatus?.no_relayer
                  ? t("relay.statusDisabled")
                  : relayStatus?.relay_bound
                    ? t("relay.statusBound")
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                ...
                {relayStatus?.relay_bound || relayStatus?.password_set ? (
                  <div
                    style={{
                      fontSize: "12px",
                      fontWeight: 600,
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px", marginTop: "4px" }}>
              <button
                type="button"
                disabled={relayConfigBusy}
                onClick={() => setRelaySettingsOpen(false)}
                style={{
                  padding: "10px 16px",
                  borderRadius: "10px",
                  border: "1px solid var(--border-color)",
                  ...
                  fontSize: "13px",
                  fontWeight: 600,
                  cursor: relayConfigBusy ? "not-allowed" : "pointer",
                }}
              >
                {t("relay.cancel")}
              </button>
              <button
                ...
                style={{
                  ...
                  borderRadius: "10px",
                  border: "none",
                  background: relayConfigBusy ? "rgba(148, 163, 184, 0.4)" : "var(--accent-color)",
                  color: "#fff",
                  fontSize: "13px",
                  fontWeight: 600,
                  cursor: relayConfigBusy ? "not-allowed" : "pointer",
...
function taskAuxBadgeStyle(attention = false): React.CSSProperties {
  return {
    ...
    color: "var(--text-secondary)",
    animation: attention ? "curvature-task-ask-user-pulse 2.2s ease-in-out infinite" : "none",
  };
}
...
function taskReplyPulseStyle(): React.CSSProperties {
  return {
    ...
    background: "#2563eb",
    animation: "curvature-bound-pulse 2.2s ease-in-out infinite",
