...
function WordPreview({ blob }: { blob: Blob }) {
  ...
  useEffect(() => {
    ...
    const fitPages = () => {
      ...
      const wrapper = container.querySelector<HTMLElement>(".curvature-docx-wrapper");
      if (!wrapper) return;
      ...
      wrapper.querySelectorAll<HTMLElement>("section.curvature-docx").forEach((page) => {
        if (page.dataset.curvatureOriginalPaddingLeft === undefined) {
          page.dataset.curvatureOriginalPaddingLeft = page.style.paddingLeft;
          page.dataset.curvatureOriginalPaddingRight = page.style.paddingRight;
        }
        page.style.setProperty("padding-left", page.dataset.curvatureOriginalPaddingLeft ?? "");
        page.style.setProperty("padding-right", page.dataset.curvatureOriginalPaddingRight ?? "");
        ...
        page.querySelectorAll<HTMLElement>("article > table").forEach((table) => {
          if (table.dataset.curvatureOriginalWidth === undefined) {
            table.dataset.curvatureOriginalWidth = table.style.width;
            table.dataset.curvatureOriginalMaxWidth = table.style.maxWidth;
            table.dataset.curvatureOriginalTableLayout = table.style.tableLayout;
          }
          table.style.setProperty("width", table.dataset.curvatureOriginalWidth ?? "");
          table.style.setProperty("max-width", table.dataset.curvatureOriginalMaxWidth ?? "");
          table.style.setProperty("table-layout", table.dataset.curvatureOriginalTableLayout ?? "");
          const columns = Array.from(table.querySelectorAll<HTMLElement>("col"));
          columns.forEach((column) => {
            if (column.dataset.curvatureOriginalWidth === undefined) {
              column.dataset.curvatureOriginalWidth = column.style.width;
            }
            column.style.setProperty("width", column.dataset.curvatureOriginalWidth ?? "");
          });
        ...
        if (!shell?.classList.contains("curvature-docx-page-shell")) {
          shell = window.document.createElement("div");
          shell.className = "curvature-docx-page-shell";
          page.before(shell);
