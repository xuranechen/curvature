...
function curvatureFSAssetPath(raw: string): string {
  const value = String(raw || "").trim();
  if (!value) {
    return "";
  }
  try {
    ...
    const knownAsset =
      ...
      pathname.startsWith("/curvature-assets/") ||
...
function showFrontendAssetMissingNotice(rawPath: string): void {
  ...
  const id = "curvature-asset-missing-notice";
...
function resourceURLFromEventTarget(target: EventTarget | null): string {
  ...
  const value =
    ...
    "";
  return curvatureFSAssetPath(value);
}
...
function dynamicImportFailureURL(reason: unknown): string {
  ...
  return curvatureFSAssetPath(match[0]);
}
...
function isNativeLauncherOrigin(): boolean {
  ...
  if (isHarmonyRuntime()) {
    return hostname === "curvature.local";
  }
  return hostname === "localhost" || hostname === "127.0.0.1" || hostname === "curvature.local";
}
...
function nativeLauncherURL(): string {
  ...
  if (isHarmonyRuntime()) {
    return "https://curvature.local/index.html";
  }
  const hostname = window.location.hostname.toLowerCase();
  if (hostname === "curvature.local") {
    return "https://curvature.local/index.html";
  }
  return "http://localhost";
}
...
function resolveTopVisibleColor(): string | null {
  ...
  const safeTopRaw = getComputedStyle(document.documentElement)
    .getPropertyValue("--curvature-safe-area-top")
    .trim();
...
function syncViewportHeight(): void {
  ...
  if (isNativeShell) {
    const imeBottom = readRootPixelVar("--curvature-ime-bottom");
    if (imeBottom > 0) {
  ...
  document.documentElement.style.setProperty("--curvature-viewport-height", `${viewportHeight}px`);
  document.documentElement.style.setProperty("--curvature-viewport-offset-top", `${viewportOffsetTop}px`);
}
...
function AppRoot() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("curvature:safe-area-updated", syncViewportHeight as EventListener);
    ...
    if (!isNativeShellRuntime()) {
      return () => {
        ...
        window.removeEventListener("curvature:safe-area-updated", syncViewportHeight as EventListener);
          ...
          try {
            ...
            const syncStatusBar = async () => {
              ...
              const color = resolveTopVisibleColor()
                || normalizeSystemBarColor(rootStyles.getPropertyValue("--curvature-system-bar-bg"), fallback);
            ...
            window.addEventListener("curvature:safe-area-updated", handleChange as EventListener);
            window.addEventListener("curvature:native-theme-changed", handleChange as EventListener);
            if (typeof media.addEventListener === "function") {
              ...
              cleanupThemeSync = () => {
                ...
                window.removeEventListener("curvature:safe-area-updated", handleChange as EventListener);
                window.removeEventListener("curvature:native-theme-changed", handleChange as EventListener);
                observer.disconnect();
              };
            } else {
    ...
    const onBackRequest = () => {
      const event = new CustomEvent("curvature:android-back-request");
