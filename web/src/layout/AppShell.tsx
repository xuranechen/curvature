...
const sidebarStyle: React.CSSProperties = {
  ...
  overflow: "auto",
  background: "var(--curvature-topbar-bg, var(--sidebar-bg))",
  display: "flex",
...
const mainStyle: React.CSSProperties = {
  ...
  overflow: "hidden",
  padding: "0",
  background: "var(--curvature-topbar-bg, var(--mobile-overlay-bg, var(--content-bg)))",
  display: "flex",
  flexDirection: "column",
  minHeight: 0,
  position: "relative",
...
export function AppShell({
  ...
  const mobileHeight = "var(--curvature-viewport-height, 100dvh)";
  ...
  const shellStyle: React.CSSProperties & {
    "--curvature-actionbar-bottom-padding"?: string;
    "--curvature-file-menu-width"?: string;
  } = {
    ...
    background: isMobile
      ? "var(--curvature-topbar-bg, var(--curvature-system-bar-bg, var(--mobile-overlay-bg, var(--content-bg))))"
      : "var(--bg-gradient-composite, var(--bg-gradient-start, #f3f4f6))",
    color: "var(--text-primary)",
    position: "relative",
    width: isMobile ? "100%" : undefined,
    maxWidth: isMobile ? "100%" : undefined,
    paddingTop: isMobile ? "var(--curvature-safe-area-top, env(safe-area-inset-top, 0px))" : undefined,
    overflow: "hidden",
    isolation: "isolate",
    boxSizing: "border-box",
    transition: "grid-template-columns 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
    "--curvature-actionbar-bottom-padding": "calc(var(--curvature-safe-area-bottom) + 12px)",
    "--curvature-file-menu-width": isMobile ? "52.5vw" : (isTablet ? "140px" : "182px"),
  };

  const mobileSidebarStyle = (side: 'left' | 'right'): React.CSSProperties => ({
    position: "fixed",
    top: "var(--curvature-safe-area-top, env(safe-area-inset-top, 0px))",
    bottom: 0,
    [side]: 0,
    width: "75vw",
    zIndex: 2000,
    background: "var(--curvature-topbar-bg, var(--mobile-sidebar-bg, var(--sidebar-bg)))",
    ...
    <div style={shellStyle} data-onboarding="shell">
      ...
      {!isMobile ? (
        <>
          <button
            type="button"
            className={`curvature-sidebar-resize-rail curvature-sidebar-resize-rail--left${physicalLeftOpen ? " is-open" : " is-closed"}`}
          ...
          {physicalRightContent ? (
            <button
              type="button"
              className={`curvature-sidebar-resize-rail curvature-sidebar-resize-rail--right${physicalRightOpen ? " is-open" : " is-closed"}`}
