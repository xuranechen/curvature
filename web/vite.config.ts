...
function buildServiceWorker(precacheUrls: string[], version: string): string {
  return `const SHELL_CACHE = "curvature-shell-${version}";
const RUNTIME_CACHE = "curvature-runtime-${version}";
...
self.addEventListener("push", (event) => {
  event.waitUntil((async () => {
    ...
    const title = payload.title || "Curvature";
    const options = {
      body: payload.body || "",
...
function readPushPayload(event) {
  ...
  try {
    ...
    return { title: "Curvature", body: event.data.text() };
  }
}
...
self.addEventListener("fetch", (event) => {
  ...
  if (pathname.startsWith("/curvature-assets/")) {
    return;
  }
  ...
  return {
    ...
    transformIndexHtml(html: string) {
      ...
      const pwaMeta = [
        ...
        '    <meta name="apple-mobile-web-app-title" content="Curvature" />',
...
function appShellExcludeAssetsPlugin() {
  let resolvedOutDir = "";
  return {
    name: "curvature-app-shell-exclude-assets",
