# Curvature Relay Server（自建中转）

一个兼容 **Curvature 官方 relay 协议**的自托管中转服务。家里运行 Curvature 的设备主动连上你公网服务器上的这个 relay，你在任何地方通过自己域名的公网地址访问家里的 Curvature——**不经过官方 relay，不装任何内网穿透工具**。

```
手机/笔记本 ── https://relay.你的域名.com/n/{node_id}/ ──▶ [relay-server]
                                                            │  WebSocket + yamux
                                                        [家里 Curvature]
```
...
参考 Curvature 客户端协议（`server/internal/relay/*`），实现了：
...
- `GET /api/token-station/userinfo` token 加油站接口（占位实现）
- `GET /api/auth/me`、`GET /login` 前端兼容端点（前端在 relay 模式下 WS 断开会探测 `/api/auth/me`，返回 200 避免误判未登录跳转，见 `web/src/App.tsx` `handleRelayWebSocketClosed`）
- `GET /nodes` 设备列表页
...
```bash
# Linux x86-64
GOOS=linux GOARCH=amd64 go build -o curvature-relay .
# 或本机直接编译（Windows 自用）
go build -o curvature-relay.exe .
```
...
- `-base` 必填，是公网访问地址（用于生成设备 WebSocket 端点和节点 URL）
...
### 3. 家里 Windows 客户端指向你的 relay

启动 Curvature 前设置环境变量：

```powershell
$env:CURVATURE_RELAY_BASE_URL = "https://relay.你的域名.com"
curvature -addr 0.0.0.0:7331
```

也可以在 `agents.json` 里写 `"relayBaseURL": "https://relay.你的域名.com"`，或直接用 `-config config.json` 传入（参考项目根目录 `config.json` 模板）。
...
- token 加油站为占位实现（`/api/token-station/userinfo` 返回空数据），不影响主功能
- WebPush 通知不经过 relay，仍需 Curvature 本地直连配置
- 未处理 `purpose=token_station` 之外的特殊绑定场景
...
AGPL-3.0。`wsframes.go` 中 WebSocket 桥接帧格式参考了 Curvature（AGPL-3.0）的 `server/internal/relay/wsconn.go`。