...
const (
	daemonEnvKey          = "CURVATURE_DAEMON"
	internalRestartEnvKey = "CURVATURE_INTERNAL_RESTART"
...
func main() {
	...
	if autoStartBoot {
		if err := prepareAutoStartEnvironment(); err != nil {
			log.Printf("[curvature/autostart] environment restore warning: %v", err)
		}
	}
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(out, "  curvature [flags] [root]\n\n")
		fmt.Fprintf(out, "Arguments:\n")
		fmt.Fprintf(out, "  root    Directory to manage. If omitted, Curvature opens without adding a directory.\n\n")
		fmt.Fprintf(out, "Flags:\n")
		flag.PrintDefaults()
		fmt.Fprintf(out, "\nExamples:\n")
		fmt.Fprintf(out, "  curvature\n")
		fmt.Fprintf(out, "  curvature /path/to/project\n")
		fmt.Fprintf(out, "  curvature --foreground\n")
		fmt.Fprintf(out, "  curvature --status\n")
		fmt.Fprintf(out, "  curvature --version\n")
		fmt.Fprintf(out, "  curvature --update\n")
		fmt.Fprintf(out, "  curvature --uninstall\n")
		fmt.Fprintf(out, "  curvature --stop\n")
		fmt.Fprintf(out, "  curvature -addr :9000 /path/to/project\n")
		fmt.Fprintf(out, "  curvature -remove /path/to/project\n")
		fmt.Fprintf(out, "  curvature <rootid> -task 12\n")
		fmt.Fprintf(out, "  curvature <rootid> -task 12 -next\n")
	}
	...
	stop := flag.Bool("stop", false, "stop the background curvature service")
	restart := flag.Bool("restart", false, "restart the background curvature service")
	statusFlag := flag.Bool("status", false, "show background service status")
	versionFlag := flag.Bool("version", false, "show version")
	updateFlag := flag.Bool("update", false, "check for and install the latest Curvature release")
	uninstallFlag := flag.Bool("uninstall", false, "print the Curvature uninstall command")
	bindRelay := flag.Bool("bind-relay", false, "start relay binding and print the relayer bind URL")
	configFlag := flag.String("config", "", "curvature startup config file; command-line flags override file values")
	...
	if internalRestart {
		log.Printf("[curvature] internal restart detected addr=%s root_arg_count=%d", *addr, flag.NArg())
	}
	...
	if *stop {
		if err := stopService(*addr, *tlsFlag, pidPath); err != nil {
			if errors.Is(err, os.ErrNotExist) {
				fmt.Fprintln(os.Stdout, "curvature service already stopped")
				return
			}
			fmt.Fprintln(os.Stderr, err.Error())
			os.Exit(1)
		}
		fmt.Fprintln(os.Stdout, "curvature service stopped")
		return
	}
	if *restart {
	...
	if !daemonMode && !internalRestart && !internalAutoStart {
		...
		if *autoStart || (!autoStartExplicit && autoStartConfigured()) {
			...
			if _, err := configureAutoStart(logPath, autoArgs); err != nil {
				fmt.Fprintf(os.Stderr, "curvature autostart warning: %v\n", err)
			}
		} else if autoStartExplicit {
			if err := removeAutoStart(); err != nil {
				fmt.Fprintf(os.Stderr, "curvature autostart disable warning: %v\n", err)
			}
		}
	}
	...
	if !*foreground && !daemonMode && !internalRestart {
		...
		rootID := ""
		fmt.Fprintln(os.Stdout, "curvature service started")
		if hasRootArg {
			absRoot, err = filepath.Abs(root)
			if err != nil {
				fmt.Fprintln(os.Stderr, err.Error())
...
func ensureStateDir() (string, error) {
	...
	dir := filepath.Join(base, "curvature")
...
func servicePaths(stateDir, addr string) (string, string, error) {
	...
	return filepath.Join(stateDir, "curvature-"+key+".pid"), filepath.Join(logDir, "curvature.log"), nil
}
...
func printVersion() {
	fmt.Fprintf(os.Stdout, "curvature version: %s\n", version)
}
...
func printUninstallCommand() {
	if runtime.GOOS == "windows" {
		...
		fmt.Fprintln(os.Stdout, "Uninstall Curvature, keeping user config and project data:")
		fmt.Fprintln(os.Stdout, "Download scripts/install.ps1 from https://github.com/a9gent/curvature, then run it with -Uninstall.")
		fmt.Fprintln(os.Stdout, "")
		fmt.Fprintln(os.Stdout, "Purge Curvature, including user config and logs:")
		fmt.Fprintln(os.Stdout, "Download scripts/install.ps1 from https://github.com/a9gent/curvature, then run it with -Uninstall -Purge.")
		return
	}
	fmt.Fprintln(os.Stdout, "Run one of the following commands manually:")
	fmt.Fprintln(os.Stdout, "")
	fmt.Fprintln(os.Stdout, "Uninstall Curvature, keeping user config and project data:")
	fmt.Fprintln(os.Stdout, `installer="${TMPDIR:-/tmp}/curvature-install.sh"`)
	fmt.Fprintln(os.Stdout, `curl -fsSL https://raw.githubusercontent.com/a9gent/curvature/main/scripts/install.sh -o "$installer"`)
...
func handleUpdateCommand(ctx context.Context, addr string, useTLS bool) error {
	if runtime.GOOS == "windows" && serverRunning(addr, useTLS) {
		return errors.New("curvature service is running. Stop it before updating:\n  curvature -stop\n  curvature -update\n  curvature")
	}
	...
	if !result.HasUpdate {
		fmt.Fprintln(os.Stdout, "curvature is already up to date")
		return nil
	}
	if result.Installed {
		fmt.Fprintln(os.Stdout, "Restart Curvature to use the new version:")
		fmt.Fprintln(os.Stdout, "  curvature -restart")
	}
	return nil
}
...
func printServiceStatus(addr string, useTLS bool, pidPath, logPath string) error {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			fmt.Fprintln(os.Stdout, "curvature status: stopped")
			fmt.Fprintf(os.Stdout, "version: %s\n", version)
			return nil
		}
		return err
	}
	fmt.Fprintln(os.Stdout, "curvature status: running")
...
func resolveServicePID(addr string, useTLS bool, pidPath string) (int, error) {
	...
	pid, err = findListeningCurvaturePID(addr)
	if err != nil {
		return 0, err
	}
...
func newHTTPClient(useTLS bool, timeout time.Duration) *http.Client {
	...
	if useTLS {
		...
		// calls connect to the local Curvature server (loopback or same machine),
...
func addManagedDir(addr string, useTLS bool, path string) (managedDirResponse, error) {
	...
	req.Header.Set("X-Curvature-Local-CLI-Token", token)
...
func handleTaskCommand(addr string, useTLS bool, rootID, taskNumberRaw, action string) error {
	...
	if rootID == "" {
		return errors.New("root id argument required: curvature <rootid> -task <task_number>")
	}
...
func openBrowser(target string) error {
	...
	if runtime.GOOS == "linux" && strings.TrimSpace(os.Getenv("DISPLAY")) == "" && strings.TrimSpace(os.Getenv("WAYLAND_DISPLAY")) == "" {
		return fmt.Errorf("%w; open this URL manually: %s; you can run `curvature -bind-relay` to get a relay binding URL and access Curvature from the public internet after binding", errBrowserUnavailable, target)
	}
	var cmd *exec.Cmd
	switch runtime.GOOS {
	case "darwin":
