...
func installPlatformAutoStart(exe string, args []string, _ string) (string, error) {
	...
	reg := exec.Command("reg.exe", "add", `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`, "/v", "Curvature", "/t", "REG_SZ", "/d", command, "/f")
	...
	return `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\Curvature`, nil
}
...
func removePlatformAutoStart() error {
	reg := exec.Command("reg.exe", "delete", `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`, "/v", "Curvature", "/f")
	reg.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
