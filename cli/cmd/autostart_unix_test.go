...
func TestLaunchAgentContentEscapesArguments(t *testing.T) {
	content := launchAgentContent("/Applications/Mind & FS/curvature", []string{"--agent-config", "/tmp/a<b.json"}, "/tmp/curvature.log")
	for _, expected := range []string{"<key>RunAtLoad</key><true/>", "/Applications/Mind &amp; FS/curvature", "/tmp/a&lt;b.json"} {
...
func TestSystemdUnitQuotesExecutableAndArguments(t *testing.T) {
	content := systemdUnitContent("/opt/Mind FS/curvature", []string{"--notify-script", `/tmp/a"b`})
	for _, expected := range []string{`ExecStart="/opt/Mind FS/curvature"`, `"/tmp/a\"b"`, "Restart=on-failure"} {
		if !strings.Contains(content, expected) {
...
func TestInstallAndRemovePlatformAutoStart(t *testing.T) {
	...
	location, err := installPlatformAutoStart("/opt/curvature", []string{"--internal-autostart"}, filepath.Join(home, "curvature.log"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if runtime.GOOS == "linux" {
		link := filepath.Join(configHome, "systemd", "user", "default.target.wants", "curvature.service")
