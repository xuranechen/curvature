...
func installPlatformAutoStart(exe string, args []string, logPath string) (string, error) {
	...
	if runtime.GOOS == "darwin" {
		path := filepath.Join(home, "Library", "LaunchAgents", "com.a9gent.curvature.plist")
	...
	unitPath := filepath.Join(configHome, "systemd", "user", "curvature.service")
	...
	wantsPath := filepath.Join(wantsDir, "curvature.service")
...
func removePlatformAutoStart() error {
	...
	paths := []string{
		filepath.Join(configHome, "systemd", "user", "default.target.wants", "curvature.service"),
		filepath.Join(configHome, "systemd", "user", "curvature.service"),
		filepath.Join(configHome, "autostart", "curvature.desktop"),
	}
...
func launchAgentContent(exe string, args []string, logPath string) string {
	...
	return "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
		...
		"<plist version=\"1.0\">\n<dict>\n" +
		"  <key>Label</key><string>com.a9gent.curvature</string>\n" +
...
func systemdUnitContent(exe string, args []string) string {
	...
	return "[Unit]\nDescription=Curvature background service\nAfter=network.target\n\n" +
