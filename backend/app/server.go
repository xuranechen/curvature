...
import (
	...
	"curvature/backend/internal/api"
	...
	"curvature/backend/internal/kanban"
	"curvature/backend/internal/notifyscript"
	"curvature/backend/internal/preferences"
	"curvature/backend/internal/relay"
	"curvature/backend/internal/scheduled"
	"curvature/backend/internal/tlsutil"
	"curvature/backend/internal/webpush"
)

const staticDirEnvKey = "CURVATURE_STATIC_DIR"
...
func autoAddExternalProjectRoots(registry *fs.Registry, prefs *preferences.Store) {
	...
	for _, projectPath := range agent.DiscoverExternalProjectPaths() {
		...
		if hasCurvatureMetadataDir(projectPath) {
			continue
		}
...
func hasCurvatureMetadataDir(projectPath string) bool {
	projectPath = strings.TrimSpace(projectPath)
	if projectPath == "" {
		return false
	}
	info, err := os.Stat(filepath.Join(projectPath, ".curvature"))
...
func resolveStaticDirFromExecutable(exe string) string {
	...
	// 本地构建布局为仓库根目录 curvature + web/dist。
	...
	// 安装布局为 <prefix>/bin/curvature + <prefix>/share/curvature/web。
	candidate = filepath.Join(filepath.Dir(exeDir), "share", "curvature", "web")
	if isFrontendStaticDir(candidate) {
		return candidate
	}
	return ""
...
// under os.UserConfigDir/curvature/ and reused across restarts.
