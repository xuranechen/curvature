...
func TestShellCommandUsesConfiguredShellOrder(t *testing.T) {
	...
	shell, _, err := shellCommand("echo ok", []ShellSpec{{Command: "definitely-not-a-curvature-shell"}, {Command: fallback}})
	if err != nil {
...
func TestShellCommandFailsWhenConfiguredShellsAreUnavailable(t *testing.T) {
	_, _, err := shellCommand("echo ok", []ShellSpec{{Command: "definitely-not-a-curvature-shell"}})
	if err == nil {
...
func TestStripShellControlEchoSuppressesSplitPowerShellLine(t *testing.T) {
	...
	if got := run.stripShellControlEchoLocked("ok\nPS C:\\Users\\me> $__curvature_status = if ($global:LASTEXITCODE"); got != "ok\n" {
...
func TestStripShellControlEchoSuppressesPowerShellWriteOutputEcho(t *testing.T) {
	run := &longShellCommand{}
	end := "__CURVATURE_CMD_END_abc__:"

	value := "real output\r\n" +
		"PS C:\\Users\\me> Write-Output ('" + end + "' + $__curvature_status)\r\n" +
		"next\r\n"
...
func TestStripShellControlEchoSuppressesWrappedPowerShellWriteOutputEcho(t *testing.T) {
	...
	if got := run.stripShellControlEchoLocked("t ('__CURVATURE_CMD_END_abc__:' + $__curvature_status)\r\nnext\r\n"); got != "next\r\n" {
...
func TestFindEndMarkerWithExitCodeIgnoresPowerShellEcho(t *testing.T) {
	...
	value := "PS C:\\Users\\me> Write-Output ('" + end + "' + $__curvature_status)\r\n" +
		"real output\r\n" +
		end + "127\r\n"
...
func TestTakeExitCodeRejectsPowerShellExpression(t *testing.T) {
	if code, ok := takeExitCode("' + $__curvature_status)\r\n"); ok {
