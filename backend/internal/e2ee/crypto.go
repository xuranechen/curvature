...
func BuildOpenProof(pairingSecret, nodeID, clientEphPK, clientNonce string) string {
	return buildProof(pairingSecret, "curvature-e2ee-open", nodeID, clientEphPK, clientNonce)
}

func BuildAcceptProof(pairingSecret, nodeID, clientEphPK, nodeEphPK, clientNonce, serverNonce string) string {
	return buildProof(pairingSecret, "curvature-e2ee-accept", nodeID, clientEphPK, nodeEphPK, clientNonce, serverNonce)
}
...
func BuildRequestProof(key []byte, method, path, ts, clientID string) string {
	...
	sum := sha256.Sum256([]byte(joinParts("curvature-request-proof", method, path, ts, clientID)))
	_, _ = h.Write(sum[:])
	return base64.StdEncoding.EncodeToString(h.Sum(nil))
}
