...
func TestManagerRecordRelatedWorktreeDoesNotOverwriteExisting(t *testing.T) {
	...
	firstPath := filepath.Join(rootDir, "..", "curvature-worktree-a")
	secondPath := filepath.Join(rootDir, "..", "curvature-worktree-b")
...
func TestManagerFallsBackToUserDataSessionDBOnSQLitePanic(t *testing.T) {
	...
	originalConfigDir := curvatureFSConfigDir
	defer func() {
		openSQLiteDB = originalOpen
		curvatureFSConfigDir = originalConfigDir
	}()
	configDir := t.TempDir()
	curvatureFSConfigDir = func() (string, error) {
