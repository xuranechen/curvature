...
import (
	...
	"curvature/backend/internal/agent"
	agenttypes "curvature/backend/internal/agent/types"
	rootfs "curvature/backend/internal/fs"
	"curvature/backend/internal/preferences"
	"curvature/backend/internal/session"
)

func TestSaveUploadedFilesDefaultsToAttachmentDirAndRenamesConflicts(t *testing.T) {
	rootDir := t.TempDir()
	root := rootfs.NewRootInfo("curvature", "curvature", rootDir)
	...
	wantFirst := filepath.ToSlash(filepath.Join(".curvature", "upload", dateDir, "demo.txt"))
	wantSecond := filepath.ToSlash(filepath.Join(".curvature", "upload", dateDir, "demo (1).txt"))
...
func TestSendCommandMessagePersistsFinalToolCallAndSuggestion(t *testing.T) {
	...
	err = service.SendMessage(context.Background(), SendMessageInput{
		RootID:  root.ID,
		Key:     created.Key,
		Content: "printf curvature-command",
		OnUpdate: func(event agenttypes.Event) {
			...
			case "final":
				...
				if len(toolCall.Content) == 0 || !strings.Contains(toolCall.Content[0].Text, "curvature-command") {
	...
	if len(candidates) != 1 || candidates[0].Name != "printf curvature-command" {
...
func TestListSessionsReturnsPinnedSnapshotWithAfterTime(t *testing.T) {
	root := rootfs.NewRootInfo("curvature", "curvature", t.TempDir())
...
func TestSearchCommandCandidatesMergesCurvatureAndShellHistory(t *testing.T) {
	rootDir := t.TempDir()
	root := rootfs.NewRootInfo("curvature", "curvature", rootDir)
	manager := session.NewManager(root)
...
func TestSearchCommandCandidatesCleansCurvatureControlHistory(t *testing.T) {
	rootDir := t.TempDir()
	root := rootfs.NewRootInfo("curvature", "curvature", rootDir)
	manager := session.NewManager(root)
	historyFile := filepath.Join(rootDir, "zsh_history")
	history := strings.Join([]string{
		": 1710000000:0;command printf '\\n%s\\n' '__CURVATURE_CMD_START_abc__'",
		": 1710000001:0;eval 'git status' </dev/null",
		": 1710000002:0;__curvature_status=$?",
		": 1710000003:0;command printf '\\n%s%s\\n' '__CURVATURE_CMD_END_abc__:' \"$__curvature_status\"",
...
func TestFileCandidateProviderSearch(t *testing.T) {
	...
	mustWriteFile(t, filepath.Join(rootDir, ".curvature", "state.json"), "a")
	...
	for _, item := range items {
		switch item.Name {
		case "node_modules/pkg/index.js", ".git/config", ".curvature/state.json", ".DS_Store":
...
func TestSwitchReadHintPathUsesRuntimeRoot(t *testing.T) {
	...
	if !strings.HasPrefix(basePath, ".curvature/") {
		t.Fatalf("base path = %q, want .curvature relative path", basePath)
	}

	worktreeRoot := filepath.Join(rootDir, ".worktree", "task-1")
	worktreePath := switchReadHintPath(manager, created.Key, worktreeRoot)
	if !strings.HasPrefix(worktreePath, "../../.curvature/") {
...
func TestRelatedFileRecordPathUsesRuntimeRoot(t *testing.T) {
	rootDir := filepath.Clean("/project/curvature")
...
func TestSessionNameRunnerRealAgent(t *testing.T) {
	if os.Getenv("CURVATURE_RUN_REAL_AGENT") != "1" {
		t.Skip("set CURVATURE_RUN_REAL_AGENT=1 to run real agent interaction test")
	}
	...
	if !ok {
		t.Skip("no runnable configured agent found (set CURVATURE_IT_AGENT_NAME)")
	}
...
func selectRunnableAgent(cfg agent.Config) (string, bool) {
	want := strings.TrimSpace(os.Getenv("CURVATURE_IT_AGENT_NAME"))
	if want != "" {
