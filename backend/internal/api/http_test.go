...
func TestRequestProofPathPreservesEscapedPathSegments(t *testing.T) {
	req := httptest.NewRequest(
		http.MethodGet,
		"/api/sessions/session-1/toolcalls/claude-task-list%3A1?root=curvature",
		nil,
	)

	got := requestProofPath(req)
	want := "/api/sessions/session-1/toolcalls/claude-task-list%3A1?root=curvature"
	if got != want {
...
func TestServeFrontendIndexRewritesRelayedAssetRefsForReleaseVersion(t *testing.T) {
	...
	req.Header.Set("X-Curvature-Relayed", "1")
	...
	if !strings.Contains(body, "/curvature-assets/index-test.js") || !strings.Contains(body, "/curvature-assets/index-test.css") {
...
func TestServeFrontendIndexKeepsLocalAssetRefsForDevVersion(t *testing.T) {
	...
	if strings.Contains(body, "/curvature-assets/") {
