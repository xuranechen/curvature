...
import (
	...
	"curvature/backend/internal/api/usecase"
	"curvature/backend/internal/commandexec"
	"curvature/backend/internal/e2ee"
	"curvature/backend/internal/fs"
	"curvature/backend/internal/githubimport"
	"curvature/backend/internal/gitview"
	"curvature/backend/internal/preferences"
	"curvature/backend/internal/relay"
...
const (
	...
	e2eeHeaderName        = "X-Curvature-E2EE"
	clientIDHeaderName    = "X-Curvature-Client-ID"
	e2eeProofHeaderName   = "X-Curvature-Proof"
	e2eeTSHeaderName      = "X-Curvature-TS"
	localCLIHeaderName    = "X-Curvature-Local-CLI-Token"
...
func isRelayedRequest(r *http.Request) bool {
	return strings.TrimSpace(r.Header.Get("X-Curvature-Relayed")) == "1"
}
...
func rewriteRelayedFrontendContent(content string) string {
	return strings.ReplaceAll(content, "./assets/", "/curvature-assets/")
}
...
func (h *HTTPHandler) handleTree(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// 目录不存在时返回空树，而非 400：前端会轮询 .curvature/plugins 等可选目录
		if errors.Is(err, os.ErrNotExist) {
...
func (h *HTTPHandler) handleRelayConfigSet(w http.ResponseWriter, r *http.Request) {
	manager := h.AppContext.GetRelayManager()
	if manager == nil {
		respondError(w, http.StatusServiceUnavailable, errServiceUnavailable("relay manager not configured"))
		return
	...
	if strings.Contains(req.RelayBaseURL, "\n") || strings.Contains(req.RelayBaseURL, "\r") {
		respondError(w, http.StatusBadRequest, errInvalidRequest("invalid relay_base_url"))
		return
	}
	status, err := manager.SetRelayBaseURL(req.RelayBaseURL)
	if err != nil {
...
func (h *HTTPHandler) handleRelayAccessPasswordSet(w http.ResponseWriter, r *http.Request) {
	manager := h.AppContext.GetRelayManager()
	if manager == nil {
		respondError(w, http.StatusServiceUnavailable, errServiceUnavailable("relay manager not configured"))
		return
...
func (h *HTTPHandler) handleRelayUnbind(w http.ResponseWriter, r *http.Request) {
	manager := h.AppContext.GetRelayManager()
	if manager == nil {
		respondError(w, http.StatusServiceUnavailable, errServiceUnavailable("relay manager not configured"))
		return
	}
	if err := manager.UnbindNode(r.Context()); err != nil {
		respondError(w, http.StatusInternalServerError, err)
		return
	}
	respondJSON(w, http.StatusOK, h.relayStatusWithE2EE(manager.Status()))
