...
func (s *Service) InstallLatest(ctx context.Context) (Status, error) {
	...
	s.updateStatus(func(st *Status) {
		st.Status = "installed"
		st.Message = "Update installed. Restart Curvature to use the new version."
	})
...
func (s *Service) installUpdate(ctx context.Context, version string, restart bool) error {
	...
	tmpDir, err := os.MkdirTemp("", "curvature-update-*")
	if err != nil {
		return err
	}
	cleanupTmpDir := true
...
func (s *Service) fetchRawText(ctx context.Context, path string, limit int64) (string, error) {
	...
	req.Header.Set("User-Agent", "curvature-update-checker")
...
func (s *Service) pickAsset(release latestRelease, version string) (releaseAsset, error) {
	...
	candidates := []string{
		fmt.Sprintf("curvature_%s_%s_%s%s", version, runtime.GOOS, runtime.GOARCH, ext),
		fmt.Sprintf("curvature_v%s_%s_%s%s", strings.TrimPrefix(version, "v"), runtime.GOOS, runtime.GOARCH, ext),
	}
...
func (s *Service) releaseAssetForVersion(version string) releaseAsset {
	...
	name := fmt.Sprintf("curvature_%s_%s_%s%s", version, runtime.GOOS, runtime.GOARCH, ext)
	return releaseAsset{
		Name:               name,
...
func (s *Service) fetchAndVerifyManifest(ctx context.Context, version string) (releaseManifest, error) {
	...
	version = normalizeTag(version)
	base := fmt.Sprintf("https://github.com/%s/releases/download/%s/curvature_%s_manifest.json", s.repo, version, version)
...
func (s *Service) downloadFile(ctx context.Context, url, dst string) error {
	...
	req.Header.Set("User-Agent", "curvature-updater")
	resp, err := s.client.Do(req)
	if err != nil {
		return err
	}
...
func relayAssetURL(name string) string {
	...
	if name == "" || strings.ContainsAny(name, `/\`) || relayDownloadBase == "" {
		return ""
	}
...
func (s *Service) canAutoUpdate() bool {
	...
	if base != "curvature" && base != "curvature.exe" {
		return false
	}
...
func (s *Service) unsupportedMessage() string {
	...
	base := strings.ToLower(filepath.Base(s.executable))
	if base != "curvature" && base != "curvature.exe" {
		return "Auto update is only available for curvature release binaries."
	}
...
func (l installLayout) destinationPaths(binName string) (string, string, string, string) {
	if l.Mode == "installed" {
		return filepath.Join(l.Prefix, "bin", binName),
			filepath.Join(l.Prefix, "share", "curvature", "agents.json"),
			filepath.Join(l.Prefix, "share", "curvature", "task_template.json"),
			filepath.Join(l.Prefix, "share", "curvature", "web")
	}
...
func latestReleaseNotesBody(text string) string {
	...
	for i := 1; i < len(lines); i++ {
		...
		if strings.HasPrefix(line, "# Curvature ") && parseReleaseNotesVersion(line) != "" {
...
func findPackageDir(root string) (string, error) {
	...
	for _, entry := range entries {
		if entry.IsDir() && strings.HasPrefix(entry.Name(), "curvature_") {
