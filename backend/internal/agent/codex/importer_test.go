...
func TestImportedCodexToolOutputFlattensExecContentBlocks(t *testing.T) {
	raw := []any{
		...
		map[string]any{
			"type": "input_text",
			"text": "ok  \tcurvature/backend/internal/agent/codex\t0.127s\n" +
				"ok  \tcurvature/backend/internal/agent/claude\t(cached)\n",
		},
	}
	...
	want := "ok  \tcurvature/backend/internal/agent/codex\t0.127s\n" +
		"ok  \tcurvature/backend/internal/agent/claude\t(cached)"
	if output != want {
