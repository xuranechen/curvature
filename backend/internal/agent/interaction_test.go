...
// Run example:
//
//	CURVATURE_RUN_REAL_AGENT=1 go test ./backend/internal/agent -run TestRealAgentInteraction -v
//
// Optional overrides:
//
//	CURVATURE_IT_AGENT_NAME=codex
func TestRealAgentInteraction(t *testing.T) {
	...
	if !ok {
		t.Skip("no runnable configured ACP agent found (set CURVATURE_IT_AGENT_NAME)")
	}
