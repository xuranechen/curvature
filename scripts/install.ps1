...
$RelayDownloadBase = "https://relay.a9gent.com/curvature-downloads"
...
function Uninstall-Curvature {
    $BinDir = Join-Path $Prefix "bin"
    $BinPath = Join-Path $BinDir "curvature.exe"
    $ShareDir = Join-Path $Prefix "share\curvature"

    Write-Host "Uninstalling curvature"
    ...
    Remove-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run" -Name "Curvature" -ErrorAction SilentlyContinue
    Write-Host "  Removed automatic startup entry."

    if ($Purge) {
        $ConfigDir = Join-Path $env:APPDATA "curvature"
        Remove-Item -Recurse -Force $ConfigDir -ErrorAction SilentlyContinue
        Write-Host "  Removed user config: $ConfigDir"
        Write-Host "  Project .curvature directories were not removed."
    } else {
        Write-Host "  User config and project .curvature data were kept."
        Write-Host "  Re-run with -Uninstall -Purge to remove user-level Curvature config and logs."
    }
    Write-Host "Done."
}
...
public static class CurvatureEnvBroadcast {
...
"@ -ErrorAction SilentlyContinue | Out-Null
        ...
        $result = [UIntPtr]::Zero
        [CurvatureEnvBroadcast]::SendMessageTimeout(
            $HWND_BROADCAST,
            $WM_SETTINGCHANGE,
            [UIntPtr]::Zero,
            "Environment",
...
if (-not $Version) {
    ...
    if ($firstLine -match '^#\s+Curvature\s+(v?[0-9]+(\.[0-9]+){1,3}[^\s]*)') {
        $Version = $Matches[1]
    }
    if (-not $Version) {
...
Write-Host "Installing curvature $Version for $OS/$Arch"
Write-Host "  Prefix: $Prefix"

# ── Download ────────────────────────────────────────────────────────────────
$Filename = "curvature_${Version}_${OS}_${Arch}.zip"
$Url      = "https://github.com/$Repo/releases/download/$Version/$Filename"
$FallbackUrl = "$RelayDownloadBase/$Filename"
$TmpDir   = Join-Path $env:TEMP ("curvature_install_" + [System.IO.Path]::GetRandomFileName())
...
try {
    ...
    $PkgDir = Join-Path $TmpDir "curvature_${Version}_${OS}_${Arch}"
    ...
    $BinSrc = Join-Path $PkgDir "curvature.exe"
    ...
    Copy-Item -Force $BinSrc (Join-Path $BinDir "curvature.exe")
    Write-Host "  Binary  -> $(Join-Path $BinDir 'curvature.exe')"
    ...
    if (Test-Path $WebSrc -PathType Container) {
        $WebDest = Join-Path $Prefix "share\curvature\web"
    ...
    Write-Host ""
    Write-Host "Done. curvature installed to $BinDir\curvature.exe"
} finally {
