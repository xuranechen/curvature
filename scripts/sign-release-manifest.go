...
func main() {
	...
	repo := flag.String("repo", "a9gent/curvature", "repository name recorded in the manifest")
...
func loadPrivateKey(value, file string) (ed25519.PrivateKey, error) {
	...
	if value == "" {
		value = strings.TrimSpace(os.Getenv("CURVATURE_RELEASE_PRIVATE_KEY"))
	}
	if value == "" {
		if path := strings.TrimSpace(os.Getenv("CURVATURE_RELEASE_PRIVATE_KEY_FILE")); path != "" {
...
func loadPublicKey(value string) (ed25519.PublicKey, error) {
	value = strings.TrimSpace(value)
	if value == "" {
		value = strings.TrimSpace(os.Getenv("CURVATURE_RELEASE_PUBLIC_KEY"))
	}
	if value == "" {
...
func artifactPaths(distDir, version string) ([]string, error) {
	patterns := []string{
		filepath.Join(distDir, fmt.Sprintf("curvature_%s_*.tar.gz", version)),
		filepath.Join(distDir, fmt.Sprintf("curvature_%s_*.zip", version)),
	}
	var paths []string
...
func manifestPath(distDir, version string) string {
	return filepath.Join(distDir, fmt.Sprintf("curvature_%s_manifest.json", version))
}
