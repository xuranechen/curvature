#!/usr/bin/env bash
# Curvature installer for macOS and Linux.
...
RELAY_DOWNLOAD_BASE="https://relay.a9gent.com/curvature-downloads"
VERSION=""
PREFIX="${HOME}/.local"
UNINSTALL=0
PURGE=0
...
config_dir() {
  case "$(uname -s)" in
    Darwin) printf '%s\n' "${HOME}/Library/Application Support/curvature" ;;
    *) printf '%s\n' "${XDG_CONFIG_HOME:-${HOME}/.config}/curvature" ;;
  esac
}

state_dir() {
  printf '%s\n' "${HOME}/.local/share/curvature"
}
...
remove_autostart() {
  ...
  case "$(uname -s)" in
    Darwin)
      rm -f "${HOME}/Library/LaunchAgents/com.a9gent.curvature.plist"
      ;;
    *)
      rm -f \
        "${config_home}/systemd/user/default.target.wants/curvature.service" \
        "${config_home}/systemd/user/curvature.service" \
        "${config_home}/autostart/curvature.desktop"
...
uninstall_curvature() {
  local bin_path="${PREFIX}/bin/curvature"
  local share_dir="${PREFIX}/share/curvature"

  echo "Uninstalling curvature"
  echo "  Prefix: ${PREFIX}"
  rm -f "$bin_path"
  ...
  if [[ "$PURGE" -eq 1 ]]; then
    ...
    echo "  Project .curvature directories were not removed."
  else
    echo "  User config and project .curvature data were kept."
    echo "  Re-run with --uninstall --purge to remove user-level Curvature config and logs."
  fi
  echo "Done."
}
...
extract_version() {
  sed -nE '1s/^[[:space:]]*#[[:space:]]+Curvature[[:space:]]+(v?[0-9]+(\.[0-9]+){1,3}[^[:space:]]*).*$/\1/p'
}
...
echo "Installing curvature ${VERSION} for ${OS}/${ARCH}"
echo "  Prefix: ${PREFIX}"

# ── Download ────────────────────────────────────────────────────────────────
FILENAME="curvature_${VERSION}_${OS}_${ARCH}.tar.gz"
...
PKG_DIR="${TMPDIR}/curvature_${VERSION}_${OS}_${ARCH}"
...
# ── Install binary ──────────────────────────────────────────────────────────
mkdir -p "${PREFIX}/bin"
install -m 0755 "${PKG_DIR}/curvature" "${PREFIX}/bin/curvature"
echo "  Binary  -> ${PREFIX}/bin/curvature"

# ── Install default agent config ────────────────────────────────────────────
if [[ -f "${PKG_DIR}/agents.json" ]]; then
  mkdir -p "${PREFIX}/share/curvature"
  install -m 0644 "${PKG_DIR}/agents.json" "${PREFIX}/share/curvature/agents.json"
  echo "  Agents  -> ${PREFIX}/share/curvature/agents.json"
fi
if [[ -f "${PKG_DIR}/task_template.json" ]]; then
  mkdir -p "${PREFIX}/share/curvature"
  install -m 0644 "${PKG_DIR}/task_template.json" "${PREFIX}/share/curvature/task_template.json"
  echo "  Templates -> ${PREFIX}/share/curvature/task_template.json"
fi

# ── Install web assets (optional) ───────────────────────────────────────────
if [[ -d "${PKG_DIR}/web" ]]; then
  WEB_DEST="${PREFIX}/share/curvature/web"
  mkdir -p "${PREFIX}/share/curvature"
  rm -rf "$WEB_DEST"
...
if command -v curvature &>/dev/null; then
  echo "Done. curvature is available at: $(command -v curvature)"
else
