...
- 回复下面展示回复所用时长
- 优化 git diff 展示效果
- 修复 Window 下关闭终端导致 curvature 被关闭
- 任务卡片显示是否开启 worktree
- 服务重启上下文窗口不丢失
...
- 优化curvature 命令行为：没有目录参数时不添加当前目录
- 服务器上可以直接通过 curvature -bind-relay 获取绑定 url
- 修复无项目时无法添加项目的问题
- 修复 Windows 下路径解决错误
...
- 增加 Android 版本
- 错误恢复：自动重试
- curvature 中已有的codex/claude 会话自动/手动同步
...
- relay 模式资源加载优化
- 关联文件移动端默认折叠
- windows 系统目录下打开 curvature 空白页
- Windows 下退出终端 curvature 退出
- 切换项目后显示最后选中会话
- agent 标记为不可用时仍然可以发送
